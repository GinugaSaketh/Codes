...
void get(int64_t n, int off, int wd, vector<int> &v)
{
...
/* taken from http://pastebin.com/NQwxv32y  */
...
int readGraph(){
     ...
     fastRead(&n);
     fastRead(&e);
     //The graph is read and its edges are unmatched by default.
...
int count_matching(int n){
  int i,j,ans;
  for(i=0;i<n;i++)match[i]=false;
  while(augmentMatching(n));
  for(i=ans=0;i<n;i++){
    for(j=i+1;j<n;j++){
      if(g[i][j]==matched)ans++;
    }
  }
  return ans;
}
...
int main()
{
	...
	for (int t = 0; t < T; t++)
	{
		...
		vector<vector<int64_t> > ad(N, vector<int64_t>(w, 0));		
		...
		for (int i = 0; i < P.size(); i++)
		{
			for (int j = i + 1; j < P.size(); j++)
			{
				...
				if (ok)
				{
					g[i][j] = g[j][i] = unmatched;
				}
			}
		}
 
		findMaximumMatching(m);
